...
const meta=JSON.parse(fs.readFileSync('lesson-meta.json','utf8')); if(meta.status!=='review'||meta.lessonId!=='clover.lesson9'||meta.itemCount!==40) err.push('lesson meta review/id/count');
...
if(err.length){console.error(err.join('\n'));process.exit(1);} console.log('Clover Lesson 9 validation OK:',D.length,'items; phrase Back Up; completed English after Check; Engine v1; student allowlist.');
